...
plt.errorbar(theta[:-1], unumpy.nominal_values(measured_contrast), yerr=unumpy.std_devs(measured_contrast), fmt='x', label='Messwerte')
...
plt.errorbar(pressure, unumpy.nominal_values(n), yerr=unumpy.std_devs(n), fmt='x', label='Messwerte')
plt.plot(x, refraction_index(x, *popt), label='Fit')
plt.grid()
...
#     print(f'{pressure[i]} & {counting_rate[i]} & {n[i]} \\\\')