...
K &= |\sin(2\phi)| \tp
\end{aquation}
This is a result from the considerations in \autoref{sec:Interference} and can be rescaled by a factor $B \in [0,1]$ to account for physical effects that reduce the contrast, like imperfect polarisation overlap, reduced coherence, etc.
...
\begin{align}
  \hat{T}_\text{Int}(\phi) \coloneqq \left(\hat T(0)\hat{P}_L + \hat T(\delta)\hat{P}_R\right) \tp
\end{align}
...
\end{align}
with them time average operator $\langle\text{...}\rangle_t$ performing a time-integral over one period (here: $T = 2\pi$) is used.\\
...
\end{align}
Time averaging a $\cos^2$ gives $\oh$, while time averaging the product of two cosines gives the cosine of their phase difference, here $\cos(\delta)$. This yields the intensity 
\begin{align}
  I \propto 1 + 2 \cos(\phi)\sin(\phi) \cos(\delta) \tp
\end{align}
From this it follows that constructive interference happens at $\delta = 0$, while destructive interference happens at $\delta = \pi$.
...
\begin{align}
  \label{eq:Delta_phi}
  \Delta \delta &= \frac{2\pi}{\lambda} \left( n \cdot \overline{\mathrm{CD}} - \overline{\mathrm{AB}}\right) \tp
\end{align}
...
\begin{align}
  \Delta \delta &= \frac{2\pi}{\lambda} \left( \frac{n - \cos(\theta - \theta^\prime)}{\cos(\theta^\prime)}\right)
\end{align}
is obtained.\\
